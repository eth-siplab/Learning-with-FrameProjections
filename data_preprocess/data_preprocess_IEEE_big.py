...
class data_loader_ieeebig(base_loader):
    def __init__(self, samples, labels, args):
        super(data_loader_ieeebig, self).__init__(samples, labels, args)
...
class data_loader_ieeebig_isoalign(base_loader_isoalign):
    def __init__(self, samples, labels, specs, FT, args):
        super(data_loader_ieeebig_isoalign, self).__init__(samples, labels, specs, FT, args)
...
def prep_domains_ieeebig_subject_large(args):
    ...
    if args.framework == 'isoalign':
        ...
        data_set = data_loader_ieeebig_isoalign(xtrain, xbpms, cwt_result, FT_result, args)
    else:
        data_set = data_loader_ieeebig(xtrain, xbpms, args)
    ...
    x = x.reshape((-1, x.shape[-1]))
    data_set_test = data_loader_ieeebig(x, y, args)
...
def prep_domains_ieeebig_subject(args):
    ...
    data_set = data_loader_ieeebig(x, y, args)
    target_loader = DataLoader(data_set, batch_size=512, shuffle=False)  # For testing keep the batch size as 512
...
def prep_domains_ieeebig_subject_sp(args):
    ...
    #######################################################
    data_set_val = data_loader_ieeebig(xtrain_fine_tuning, xbpms_fine_tuning, args)
    val_loader = DataLoader(data_set_val, batch_size=args.batch_size, shuffle=False)
    #
    data_set_train = data_loader_ieeebig(xtrain_training, xbpms_training, args)
