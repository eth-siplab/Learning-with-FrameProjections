...
class data_loader_clemson_isoalign(base_loader_isoalign):
    def __init__(self, samples, labels, specs, FT, args):
        super(data_loader_clemson_isoalign, self).__init__(samples, labels, specs, FT, args)
    ...
    def __getitem__(self, index):
        ...
        # sample --> (1, 480), spec --> (48, 480, 1), FT --> (1, 241)         
...
def prep_domains_clemson_subject_large(args):
    ...
    if args.framework == 'isoalign':
        ...
        wavelet_transform = WaveletTransform(wavelet='cmor1-1', fs=15)
        ...
        FT_transform = FourierTransform(fs=15)
        FT_result = FT_transform.compute_FT(xtrain)
        data_set = data_loader_clemson_isoalign(xtrain, xbpms, cwt_result, FT_result, args)
        # train_sampler = DistributedSampler(data_set)
        # source_loader = DataLoader(train_sampler, batch_size=args.batch_size, shuffle=False)
    else:
    ...
    data_set_target = data_loader_clemson(xtest, ybpms, args)
    # target_loader = DataLoader(data_set_target, batch_size=512, shuffle=False)  # For testing keep the batch size as 512
...
def prep_domains_clemson_subject(args):
    ...
    source_domain_list = random.sample(source_domain_list, 9) # 3-fold for fine-tuning
    ...
    data_set = data_loader_clemson(balanced_signals_array, balanced_classes, args)
    source_loader = DataLoader(data_set, batch_size=args.batch_size, shuffle=False)
