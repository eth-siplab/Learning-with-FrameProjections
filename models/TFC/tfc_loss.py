...
class TFC_Loss(torch.nn.Module):
    def __init__(self, args):
        super(TFC_Loss, self).__init__()
        self.device = args.cuda
        self.nt_xent_criterion = NTXentLoss_poly(self.device, args.batch_size, 0.2, True) # device, 128, 0.2, True
        
    def forward(self, z_t, z_t_aug, z_f, z_f_aug, h_t, h_t_aug, h_f, h_f_aug):
        loss_t = self.nt_xent_criterion(h_t, h_t_aug)
        loss_f = self.nt_xent_criterion(h_f, h_f_aug)
        l_TF = self.nt_xent_criterion(z_t, z_f) # this is the initial version of TF loss

        l_1, l_2, l_3 = self.nt_xent_criterion(z_t, z_f_aug), self.nt_xent_criterion(z_t_aug, z_f), self.nt_xent_criterion(z_t_aug, z_f_aug)
