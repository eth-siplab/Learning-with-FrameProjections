...
from .models_nc import ResNet1D
...
class Projector(nn.Module):
    def __init__(self, model, bb_dim, prev_dim, dim):
        ...
        if model == 'SimCLR':
            ...
            self.projector = nn.Sequential(nn.Linear(128, dim))
        elif model == 'MTM':
            self.projector = nn.Sequential(nn.Linear(bb_dim, prev_dim),
                                           nn.BatchNorm1d(prev_dim),
                                           nn.ReLU(inplace=True),
            ...
            # self.projector = nn.Linear(bb_dim, prev_dim)
            self.projector = nn.Sequential(nn.Linear(bb_dim, prev_dim),
                                           nn.ReLU(inplace=True),
                                           nn.Linear(prev_dim, dim))
        else:
...
class TFC_backbone(nn.Module):
    def __init__(self, args, output_dim):
        ...
        if args.dataset == 'ieee_small':
            args.TSlength_aligned = 200
        elif args.dataset in ['ucihar', 'clemson']:
            args.TSlength_aligned = 128
        elif args.dataset in ['hhar', 'usc']:
            args.TSlength_aligned = 100
        elif args.dataset in ['chapman', 'cpsc']:
            args.TSlength_aligned = 1000
        
        self.out_dim = output_dim
        
        self.encoder_layers_t = TransformerEncoderLayer(args.TSlength_aligned, dim_feedforward=2*args.TSlength_aligned, nhead=2, )
        self.transformer_encoder_t = TransformerEncoder(self.encoder_layers_t, 2)
        # self.transformer_encoder_t = ResNet1D(in_channels=args.n_feature, base_filters=32, kernel_size=5, stride=2, groups=1, n_block=args.block, n_classes=args.n_class, downsample_gap=2, increasefilter_gap=4, output_dim=args.out_dim, backbone=True)
    
        self.encoder_layers_f = TransformerEncoderLayer(args.TSlength_aligned, dim_feedforward=2*args.TSlength_aligned,nhead=2,)
        self.transformer_encoder_f = TransformerEncoder(self.encoder_layers_f, 2)
        # self.transformer_encoder_f = ResNet1D(in_channels=args.n_feature, base_filters=32, kernel_size=5, stride=2, groups=1, n_block=args.block, n_classes=args.n_class, downsample_gap=2, increasefilter_gap=4, output_dim=args.out_dim, backbone=True)
    ...
    def forward(self, x, x_f):
        ...
        # _, x = self.transformer_encoder_t(x)
        # _, x_f = self.transformer_encoder_f(x_f)
        # return x.unsqueeze(1), x_f.unsqueeze(1)
        return x, x_f